...
////////////////////////////////////////////////////////////////
extern pthread_t tid_write; //erxetai apo to bench.c kai periexei to id toy nhmatos ths write

int wr = 1; //arxikopoieitai sto 1 epeidi stin db_open_ex benei prvta to nhma read

pthread_mutex_t mutex = PTHREAD_MUTEX_INITIALIZER; //arxikopoihsh mutex
pthread_cond_t condition = PTHREAD_COND_INITIALIZER; //arxikopoihsh condition
////////////////////////////////////////////////////////////////
...
////////////////////////////////////////////////////////////////
		...
		pthread_mutex_lock(&mutex); //kleidvnetai o parakatv kvdikas
		if(pthread_equal(pthread_self(), tid_write)){ //elexei an to nhma pou kaleitai ekeinh thn stigmh einai to write
			wr = 0; //an to parapanv isxyei to wr ginetai 0 pou shmainei oti trexei to nhma write
		}
...
////////////////////////////////////////////////////////////////		
		...
		while(wr == 1){ //oso vlepei to wr na einai 1, dhladh na trexei nhma read
			//an tipvthei auto epivevaivnei oti stin arxh exei klhthei to nhma read
			printf("........I AM WAITIMG(CurrentThreadId: %d)........\n", pthread_self());
			pthread_cond_wait(&condition,&mutex); //kanei wait
		}
...
////////////////////////////////////////////////////////////////
		...
		pthread_mutex_unlock(&mutex); //xekleidvnetai o parapanv kvdikas
...
////////////////////////////////////////////////////////////////
		...
		wr = 2; //otan teleivsei to prvto write ta nhmata read prepei na bainoun synexeia sthn open gi auto to wr paramenei 0
		if (wr == 2){
			pthread_cond_signal(&condition); //to nhma write jypnaei to nhma read
			printf("........THREAD READ IS AWAKE AND WAITTING[if the thread exists]........\n");
		}
		printf("____CurrentThreadId: %d\n", pthread_self());
